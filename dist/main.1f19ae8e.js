...
var checkLocalStorage = function checkLocalStorage() {
  return localStorage.getItem("currentTask.list") === null || localStorage.getItem("currentTask.list" === "");
};
...
var createNewTask = function createNewTask(task) {
  var taskTemplate = document.querySelector("#task-template");
  var clonedList = document.importNode(taskTemplate.content, true);
  var taskTxt = clonedList.querySelector(".task-txt");
  taskTxt.innerText = task;
  taskList.appendChild(clonedList);
};
var deleteTask = function deleteTask(task) {
  task.remove();
  var taskToBeDeleted = currentTasks.indexOf(task.querySelector(".task-txt").innerText);
